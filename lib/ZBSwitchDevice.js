...
class ZBSwitchDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode }) {
    ...
    const { startupOnOff } = await zclNode.endpoints[this.getClusterEndpoint(ROBBSpecificOnOffCluster)].clusters[ROBBSpecificOnOffCluster.NAME].readAttributes('startupOnOff');
    const { switchType } = await zclNode.endpoints[this.getClusterEndpoint(ROBBSpecificBasicCluster)].clusters[ROBBSpecificBasicCluster.NAME].readAttributes('switchType');
