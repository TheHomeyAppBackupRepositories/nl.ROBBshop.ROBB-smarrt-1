    ...
    if (this.hasCapability('onoff')) {
      ...
      this.registerReportListener('BASIC', 'BASIC_REPORT', report => {
        ...
        if (report && report.hasOwnProperty('Value')) {
          if (this.hasCapability('onoff')) this.setCapabilityValue('onoff', report['Value'] > 0).catch(this.error);
          if (this.hasCapability('dim')) this.setCapabilityValue('dim', report['Value'] / 99).catch(this.error);
        }
        // BASIC_REPORT V2
        if (report && report.hasOwnProperty('Current Value')) {
          if (this.hasCapability('onoff')) this.setCapabilityValue('onoff', report['Current Value'] > 0).catch(this.error);
          if (this.hasCapability('dim')) this.setCapabilityValue('dim', report['Current Value'] / 99).catch(this.error);
        }
      });
    }
