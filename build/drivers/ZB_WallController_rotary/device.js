...
module.exports = class ZB_WallController_rotary extends ZigBeeDevice {
  ...
  _levelCommandHandler(command, endpoint, payload) {
    // if (command !== 'on' && command !== 'off') throw new Error('invalid_onoff_type');
    this.log('_levelCommandHandler |', command, endpoint, payload, payload.level / 255, payload.transitionTime);
    this.triggerFlow({
      id: 'wall_controller_dim_changed',
      tokens: { dim: payload.level / 255 },
      state: null,
    })
      .then(() => this.log('flow was triggered: wall_controller_dim_changed, dim level', payload.level / 255))
  ...
  _colorCommandHandler(command, endpoint, payload) {
    ...
    if (command === 'moveToHue') {
      ...
      this.triggerFlow({ id: 'wall_controller_hue_changed', tokens: { hue: payload.hue / 224 }, state: {} })
        .then(() => this.log('flow was triggered: wall_controller_hue_changed, hue level', payload.hue / 224))
